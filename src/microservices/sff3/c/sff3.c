    ...
    while ( 1 ) {
        ...
        for ( int i = 0; i < nb_rx; i++ ) {
            ...
            if ( f_id != END_OF_STREAM ) {
                ...
                bool populated_frame = packet_valid_points > 0 && packet_points > 0 && packet_points <= POINTS_PER_PACKET && packet_payload_len == packet_points * sizeof( struct point_tx );
            ...
            if ( unlikely( f_id != current_frame_id ) ) {
                ...
                if ( current_frame_id != END_OF_STREAM && logged_frames < K_FRAMES ) {
                    ...
                    uint32_t expected_packets = ( frame_valid_points > 0 ) ? ( frame_valid_points + POINTS_PER_PACKET - 1 ) / POINTS_PER_PACKET : 1;
